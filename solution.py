...
# Ensure unittest.patch("day_01_llm_api_foundation.template...") resolves.
if "." in __name__:
    package_name, module_name = __name__.rsplit(".", 1)
    package = sys.modules.get(package_name)
    if package is not None:
        setattr(package, module_name, sys.modules[__name__])
...
def batch_compare(prompts: list[str]) -> list[dict]:
    ...
    for prompt in prompts:
        ...
        result_with_prompt = dict(result)
        result_with_prompt["prompt"] = prompt
...
def format_comparison_table(results: list[dict]) -> str:
    ...
    for result in results:
        rows.append(
            [
                ...
                f"{result.get('gpt4o_latency', 0):.3f}",
                f"{result.get('mini_latency', 0):.3f}",
            ]
        )
    ...
    for row_index, row in enumerate(rows):
        formatted_row = " | ".join(row[column].ljust(column_widths[column]) for column in range(len(headers)))
        formatted_rows.append(formatted_row)
        if row_index == 0:
            separator = "-+-".join("-" * column_widths[column] for column in range(len(headers)))
